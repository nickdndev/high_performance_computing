...
double CalcMatMulTime_kij_opt_omp(double *A, double *B, double *C, size_t N) {
  struct timeval start, end;
  double r_time = 0.0;
  size_t i, j, k;
...
int main() {
  ...
  average_runtime = 0.0;
  for (int n = 0; n < NRuns; n++) {
    runtimes[n] = CalcMatMulTime_kij_opt_omp(&A[0], &B[0], &C[0], N);
    printf("runtime %lf seconds\n", runtimes[n]);
    average_runtime += runtimes[n] / NRuns;
  }
  printf("average runtime kij opt omp %lf seconds\n", average_runtime);
  printf("---------------------------------\n");
